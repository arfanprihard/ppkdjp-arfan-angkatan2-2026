...
<div class="card">
    ...
    <form action="{{ route('locker.store') }}" method="POST" class="px-5 pb-4">
        @csrf
        <div class="mb-3">
            <label>Locker Name *</label>
            <input type="text" class="form-control @error('locker_name') is-invalid @enderror"
                placeholder="Contoh: L-001" name="locker_name" value="{{ old('locker_name') }}" required>
            @error('locker_name')
        ...
        <div class="mb-3">
            ...
            <select name="major_name" class="form-select @error('major_name') is-invalid @enderror" required>
                ...
                @foreach (['Web Programming', 'Content Creator', 'Teknisi Jaringan'] as $major)
                    <option value="{{ $major }}" {{ old('major_name') == $major ? 'selected' : '' }}>
                        {{ $major }}
                    </option>
                @endforeach
            </select>
