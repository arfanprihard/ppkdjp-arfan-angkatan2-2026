...
import { X, AlertCircle, RefreshCw, Plus, Search, Calendar } from "lucide-react";
...
const CreateReservationModal = ({ rooms, onClose, onSaved, prefilledCheckIn }) => {
  ...
  useEffect(() => {
    const fetchAllReservations = async () => {
      try {
        ...
        console.error("Gagal mengambil data reservasi untuk ketersediaan kamar", err);
      }
    };
  ...
  useEffect(() => {
    ...
    const delayDebounce = setTimeout(async () => {
      ...
      try {
        const res = await api.get(`/api/guests?search=${encodeURIComponent(guestSearch)}`);
        if (res.data.success) {
      ...
      <div className="w-full max-w-2xl bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl overflow-hidden my-8 animate-in fade-in zoom-in-95 duration-200">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
            ...
            {selectedGuest ? (
              ...
              <div className="space-y-2.5">
                ...
                {guestSearch.trim().length < 2 && defaultGuests.length > 0 && (
                  <div className="pt-1">
                    <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 mb-1.5">Tamu Terbaru (Pilih Cepat):</p>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="space-y-1">
              <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 block">Jumlah Dewasa *</label>
              <input
                type="number"
                min={1}
                required
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 block">Jumlah Anak</label>
              <input
                type="number"
                min={0}
                value={numChildren}
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 block">Channel Booking *</label>
              <select
                value={channel}
          ...
          {channel === "ota" && (
            <div className="space-y-1 animate-in slide-in-from-top-2 duration-200">
              <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 block">Nama OTA *</label>
              <input
                type="text"
                required
          ...
          {/* Special Requests */}
          <div className="space-y-1">
            <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 block">Permintaan Khusus (Opsional)</label>
            <textarea
              rows={2}
