...
// Circular Progress Ring SVG (Light Theme)
    ...
    <div className="flex flex-col items-center justify-center py-2">
      <div className="relative flex items-center justify-center">
        ...
        <div className="absolute flex flex-col items-center">
          <span className="text-3xl font-black text-zinc-800 leading-none">
            {rate}%
          </span>
...
// Custom interactive CSS-based Revenue Chart (Light Theme)
const RevenueChart = ({ data, availableYears = [], selectedYear, onYearChange }) => {
  ...
  return (
    <div className="bg-white border border-zinc-200 rounded-2xl p-6 hover:border-blue-200 transition-all duration-300 shadow-sm relative overflow-hidden group">
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          <p className="text-[10px] font-extrabold uppercase tracking-widest text-zinc-400">
            Statistik Finansial
          </p>
          <h3 className="text-base font-extrabold text-zinc-800 mt-0.5">
            Pendapatan Bulanan
          </h3>
        </div>
        ...
        <div className="flex items-center gap-2">
          {availableYears.length > 0 && (
            <select
              ...
              className="bg-white border border-zinc-200 rounded-xl px-3 py-1.5 text-xs text-zinc-700 outline-none cursor-pointer focus:border-blue-400 font-semibold"
            >
          ...
          <div className="flex items-center gap-1.5 bg-slate-50 px-2.5 py-1.5 rounded-lg border border-zinc-200 text-[10px] font-bold text-zinc-500">
      ...
      {/* Chart Canvas */}
      <div className="relative h-60 flex items-end justify-between gap-3 pt-8 border-b border-zinc-100 pb-1">
        {/* Y-Axis Gridlines */}
        <div className="absolute inset-0 flex flex-col justify-between pointer-events-none text-[9px] text-zinc-400 font-bold tracking-wider">
          <div className="border-t border-zinc-100 w-full pt-1 flex justify-between">
          ...
          {data.map((d, index) => {
            ...
            return (
              <div key={index} className="flex flex-col items-center flex-1 h-full justify-end group/bar relative">
                {/* Tooltip on hover */}
                <div className="absolute bottom-full mb-2 opacity-0 scale-95 group-hover/bar:opacity-100 group-hover/bar:scale-100 transition-all duration-200 pointer-events-none z-30 bg-white border border-zinc-200 px-3 py-1.5 rounded-xl text-center shadow-md min-w-[120px]">
                  <p className="text-[9px] font-bold uppercase text-zinc-400 tracking-wider">{d.month}</p>
                  <p className="text-xs font-black text-blue-600 mt-1">{formatRupiah(d.total)}</p>
                </div>

                {/* Bar */}
                <div
                  style={{ height: `${Math.max(pct, 4)}%` }}
                  className="w-full max-w-[20px] sm:max-w-[28px] rounded-t-lg bg-blue-600 hover:bg-blue-700 transition-all duration-350 ease-out relative cursor-pointer overflow-hidden shadow-sm"
                >
                  <div className="absolute top-0 left-0 right-0 h-1/2 bg-white/10" />
                </div>

                {/* X-Axis Month label */}
                <span className="text-[9px] sm:text-[10px] font-bold text-zinc-400 mt-2.5 group-hover/bar:text-zinc-600 transition-colors whitespace-nowrap">
                  {d.month.split(" ")[0]}
                </span>
              </div>
            );
...
const AdminDashboard = ({ data, loading, onYearChange }) => {
  ...
  const quickActions = [
    ...
    },
    {
      ...
      icon: ShieldAlert,
      path: "/users",
      color: "hover:border-rose-300 hover:text-rose-600",
    },
    {
      label: "Status Kamar",
      icon: Layers,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-base font-bold text-zinc-800">Overview Admin</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        {/* Occupancy Ring */}
        <div className="bg-white border border-zinc-200 rounded-2xl p-5 flex flex-col items-center justify-center hover:border-blue-200 transition-all duration-300 relative overflow-hidden group shadow-sm">
          {loading ? (
            ...
            <>
              ...
              <p className="text-xs text-zinc-500 mt-1 font-medium">
                <span className="text-zinc-800 font-bold">{data?.occupied_rooms ?? 0}</span> dari <span className="text-zinc-500 font-bold">{data?.total_rooms ?? 0}</span> kamar terisi
              </p>
            </>
          )}
        </div>
