...
import RoomTypeFormModal from "../components/rooms/RoomTypeFormModal";
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-bold text-zinc-800">
              {activeTab === "rooms" ? "Room Board" : "Tipe Kamar"}
            </h2>
            <p className="text-[11px] text-zinc-500">
              {activeTab === "rooms"
                ? (loading ? "Memuat..." : `${rooms.length} kamar total - ${filtered.length} ditampilkan`)
                : `${roomTypes.length} tipe kamar tersedia`}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={activeTab === "rooms" ? fetchRooms : fetchRoomTypes} disabled={loading}
      ...
      {activeTab === "rooms" ? (
          ...
          <div className="flex flex-wrap gap-3">
            ...
            {hasActiveFilters && (
              <button onClick={() => { setSearch(""); setFilterStatus("all"); setFilterFloor("all"); setFilterType("all"); }}
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-zinc-300 bg-white text-zinc-650 hover:text-rose-600 hover:border-rose-300 text-xs font-medium transition-all cursor-pointer shadow-xs">
        ...
        <RoomTypesTab roomTypes={roomTypes} onRefresh={fetchRoomTypes} />
      )}
      ...
      {showAddTypeModal && (
        <RoomTypeFormModal type={null} onClose={() => setShowAddTypeModal(false)}
